...
from collage_diffusion.translations import createRequest
input_dir = "image_layers/test_WHITE/layers"
temp_dir = "process_tmp"
output_dir = "output/20240415"
...
import os
print('pwd:'+os.getcwd())
model_path_diffusion = "/usr/src/app/backend/collage_diffusion/stable-diffusion-2-1-base"
...
def display(img):
    ...
    # img.save(output_dir + f'/{project_id}_{format_time}_{index}_directsave.png', bbox_inches='tight')
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
...
def image_resize(input_dir, temp_dir):
    ...
    for filename in os.listdir(pic_dir):
        ...
        width_crop = int(length * img.size[0] / max_size)
        height_crop = int(length * img.size[1] / max_size)
        ...
        new_img = new_img.resize((length, length), Image.Resampling.LANCZOS)
    ...
    def preprocess_layers(
        ...
        for layer in layers:
            ...
            image_layer = ImageLayer(
                ...
                pos=(
                    int(length * layer["transform"]["position"]["x"]),
                    int(length * layer["transform"]["position"]["y"]),
                ),
                scale=length * layer["transform"]["scale"],
...
def all_collage(experiment_name, collage_prompt, layer_list, ca, textual_inversion, device_id,
    ...
    ###############################
    pipe = StableDiffusionPipeline.from_pretrained(model_path_diffusion, torch_dtype=torch.float32).to(device)
...
def import_scene(temp_dir):
    ...
    for layer_id in np.arange(len(source_files)):
        ...
        translated = createRequest(layer_str)
        prompt = f'{prompt} and {translated}'
        layer_list.append(ImageLayer(layer_image, (0, 0), translated, ftc=None))
        time.sleep(5)
